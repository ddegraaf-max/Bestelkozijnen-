...
function postgresStore() {
  ...
  return {
    ...
    async createRequest({ userId, elementen, klant }) {
      // Uniek, datum-gebaseerd nummer (JJJJMMDD-XXXX) — altijd nieuw, ook na een
      // redeploy en zonder persistente teller. Geen "WEB".
      const d = new Date();
      const ymd = `${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, '0')}${String(d.getDate()).padStart(2, '0')}`;
      let ref = `${ymd}-${Math.floor(1000 + Math.random() * 9000)}`;
      for (let i = 0; i < 20; i++) {
        const { rows: ex } = await pool.query('SELECT 1 FROM requests WHERE ref=$1 LIMIT 1', [ref]);
        if (!ex.length) break;
        ref = `${ymd}-${Math.floor(1000 + Math.random() * 9000)}`;
      }
      const r = {
...
function jsonStore() {
  ...
  return {
    ...
    createRequest({ userId, elementen, klant }) {
      // Uniek, datum-gebaseerd nummer (JJJJMMDD-XXXX), geen "WEB" en geen
      // resetbare teller.
      const d = new Date();
      const ymd = `${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, '0')}${String(d.getDate()).padStart(2, '0')}`;
      let ref = `${ymd}-${Math.floor(1000 + Math.random() * 9000)}`;
      while (db.requests.some(r => r.ref === ref)) ref = `${ymd}-${Math.floor(1000 + Math.random() * 9000)}`;
      const req = {
        id: id(), ref, userId, elementen, klant,
        status: 'ontvangen',
